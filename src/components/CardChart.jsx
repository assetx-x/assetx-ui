...
const CardChart = ({ config }) => {
  ...
  return (
    <div className="flex flex-wrap justify-center">
      <div
        className="w-full max-w-xl bg-white border border-gray-200 rounded-lg shadow">
        <div className="px-5 pb-5" style={{display: "flex", justifyContent: "center"}}>
          {useLogo ? (<img src={Logo} className=" h-16 w-auto pt-6" alt="logo" />) :
            <h5 className="pt-6 text-xl font-semibold tracking-tight text-gray-900">{name}</h5>}
        </div>
