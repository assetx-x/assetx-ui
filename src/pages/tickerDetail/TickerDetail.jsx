...
const TickerDetail = () => {
  ...
  function formatRates(obj) {
    return {
      ...
      fundamental_ratios: {
        name: "Ratios",
        value: obj.graphing_data?.fundamental_ratios?.rating,
      },
      fundamental_cash_flow: {
        name: "Cash Flow",
        value: obj.graphing_data?.fundamental_cash_flow?.rating,
      },
      macro: { name: "Macro", value: obj.graphing_data?.macro?.rating },
      momentum_fast: {
        name: "Fast",
        value: obj.graphing_data?.momentum_fast?.rating,
      },
      trend_following: {
        name: "Trend Following",
        value: obj.graphing_data?.trend_following?.rating,
      },
    };
  }
  ...
  function formatRelativeHistoricalExplainingData(
    key = context.selectedRatingData
  ) {
    ...
    charts: [
      {
        subtitle: "Relative",
        description:
          data.graphing_data?.composite?.forecast_information?.sentence,
        type: "WaterfallChart",
        rates: formatRates(data),
        data: formatRelativeHistoricalExplainingData(
          context.selectedRatingData
        ),
      },
      {
        subtitle: "Absolute",
        type: "HistoricalExplainerChart",
        data: formatHistoricalExplainingData(context.selectedRatingData),
      },
    ],
  };
  ...
  const forecastChartConfig = {
    chart: {
      ...
      data: data.graphing_data?.composite?.forecast_information,
    },
  };
  const tabsConfig = {
    type: "underline",
    tabs: [
      {
        name: "Forecast",
        content: <DetailView config={forecastChartConfig} />,
      },
      {
        name: "Historical Explainer",
        content: <StackedView config={historicalExplainerConfig} />,
      },
    ],
  };
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between">
              <div className="flex flex-col md:flex-row md:items-center">
                <div className="flex items-center justify-center w-168 h-168 mb-4 md:mb-0 md:mr-4">
                  <img
                    src={data?.header_information?.company_logo}
                    alt={data?.header_information?.company_name}
                ...
                <div className="flex flex-col">
                  <h1 className="text-4xl pl-6">
                    {data?.header_information?.company_name}
                  </h1>
